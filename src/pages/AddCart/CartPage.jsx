            ...
            <tbody>
              {store.map((items, index) => (
                <tr className="hover:bg-gray-100 border-b" key={index}>
                  ...
                  <td className="px-3 py-3">{items.price * (items.count || 1)}
</td>
                  <td className="px-3 py-3">{items.count || 1}</td>
                  <td className="px-3 py-3">
                    <div className="inline-block bg-red-500 p-2 rounded hover:cursor-pointer" onClick={()=> dispatchFun(removeItem(index)) }>
